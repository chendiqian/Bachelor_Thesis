    ...
    case 'a'
        ...
        for j = 1: numel(zone)
            ...
            if new_position(1) >= zone(row, column).corner(1) && ...
                ...
                if (new_position(1) == zone(row, column).corner(1) || new_position(1) == zone(row, column).corner(3)) ...
                        && new_position(2) ~= zone(row, column).corner(2)+3 && new_position(2) ~= zone(row, column).corner(2)+5
                    new_task(priority).edge = 1; % on the edge
                end
                break;
            end
        end
